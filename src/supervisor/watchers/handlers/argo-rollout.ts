...
export async function argoRolloutWatchHandler(
  ...
  if (rollout.spec?.workloadRef && rollout.metadata?.namespace) {
    ...
    switch (rollout.spec.workloadRef.kind) {
      ...
      case 'Deployment': {
        const deployResult =
          await retryKubernetesApiRequest(() => k8sApi.appsClient.readNamespacedDeployment(workloadName, namespace));
        rollout.spec.template = deployResult.body.spec?.template;
        break;
      }
      case 'ReplicaSet': {
        const replacaSetResult =
          await retryKubernetesApiRequest(() => k8sApi.appsClient.readNamespacedReplicaSet(workloadName, namespace));
        rollout.spec.template = replacaSetResult.body.spec?.template;
        break;
      }
      case 'PodTemplate': {
        const podTemplateResult =
          await retryKubernetesApiRequest(() => k8sApi.coreClient.readNamespacedPodTemplate(workloadName, namespace));
